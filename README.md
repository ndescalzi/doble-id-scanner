...
Scan your **React**, **Vue**, **Angular** and **HTML** projects for duplicate `id` attributes — no installation required.

## Usage

```bash
# Scan current directory (default)
npx duplicate-id-scanner

# Scan a specific folder
npx duplicate-id-scanner ./src

# The path is always optional and can go anywhere in the command
npx duplicate-id-scanner --file ./src
npx duplicate-id-scanner ./src --file --json
```
...
| `--file` | Save the report to `idScan.md` (or `idScan.json` with `--json`). Only creates the file if duplicates are found. |
...
| `--all` | Show all found ids, not just duplicates |
| `--help`, `-h` | Show help |
...
# Save report as idScan.md (only if duplicates found)
npx duplicate-id-scanner --file

# Save report as idScan.json
npx duplicate-id-scanner --file --json

# Include dynamic id bindings (JSX id={expr}, Vue :id="expr")
npx duplicate-id-scanner --dynamic

# Full combo: specific folder, dynamic ids, save as JSON file
npx duplicate-id-scanner ./src --dynamic --file --json

# Exclude build output dirs
npx duplicate-id-scanner --no-build

# Flags and path can go in any order
npx duplicate-id-scanner --file --dynamic ./src
npx duplicate-id-scanner ./src --file --dynamic
```
...
────────────────────────────────────────────────────
Summary: 2 duplicate id(s) found across 24 file(s) scanned.
...
Dynamic ids are excluded by default because their actual value is unknown at scan time — two components using `id={props.id}` are not necessarily duplicates. Use `--dynamic` when you want to audit all id bindings regardless.
...
```yaml
# GitHub Actions example — fail the build if duplicate IDs are found
- name: Check for duplicate IDs
  run: npx duplicate-id-scanner ./src --dynamic
```

## Why does this matter?

- Duplicate `id`s break **accessibility** (screen readers, ARIA relationships)
